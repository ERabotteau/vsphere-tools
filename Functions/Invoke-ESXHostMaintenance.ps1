...
#
# -----------------------------------------------------------------------------
#region variables modifiables
Function Invoke-ESXHostMaintenance()
{
    [CmdletBinding()]
    param(
    ...
    process {
        ...
        show-MyLogger "Server is now in maintenance mode" -Color Yellow
        show-MyLogger "execute needed operations then reboot host manually" -Color Yellow
        show-MyLogger "Script will continue only if server is down" -Color Yellow
        ...
        show-MyLogger "... Do whatever you need to do to this Esx Host. this script will wait for esx is back..."
 
        # Wait for server to reboot
        do {
        sleep 60
        $ServerState = (get-vmhost $ESXHostName).ConnectionState
        show-MyLogger "Waiting for host ..." -Color Yellow
        }
